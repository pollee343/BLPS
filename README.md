...
Вариант №666: МТС — связь и экосистема цифровых сервисов — https://mts.ru. Бизнес-процесс: работа с разделами основного сайта — мобильная связь (в т.ч. пополнение счёта), для дома, сервисы и подписки, финансы.
...
4. Разработать приложение на базе Spring Boot, реализующее описанный на предыдущем шаге бизнес-процесс. Приложение должно использовать СУБД PostgreSQL для хранения данных, длявсех публичных интерфейсов должны быть разработаны REST API.
...
3. Реализовать разработанную модель разграничений доступа к операциям бизнес-логики на базе Spring Security + JAAS. Информацию об учётных записах пользователей необходимо сохранять в реляционую базу данных, для аутентификации использовать JWT.
